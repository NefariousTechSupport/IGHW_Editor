...
public class StreamHelper : BinaryReader
{
	...
	public void WriteUInt32(uint data, Endianness endianness)
	{
		...
		{
			writeBuffer = BitConverter.GetBytes(data).Reverse().ToArray();	//Bad Code;
		}
		else
		{
			writeBuffer = BitConverter.GetBytes(data);
		}
		BaseStream.Write(writeBuffer, 0x00, 0x04);
	} 
		
	public void WriteUInt32WithOffset(uint data, uint offset) => WriteUInt32WithOffset(data, offset, _endianness);
	public void WriteUInt32WithOffset(uint data, uint offset, Endianness endianness)
	{
		BaseStream.Seek(offset, SeekOrigin.Begin);
		WriteUInt32(data, endianness);
	}
